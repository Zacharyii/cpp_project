...
int main(int argc,char *argv[])
{
    if(argc<3)
    {
        ...
        cout<<"Example:/project/tools/bin/procctl 10 /usr/bin/tar zcvf /tmp/tmp.tgz /usr/include\n";//调度程序本身 时间间隔 被调度程序 参数
        cout<<"Example:/project/tools/bin/procctl 60 /project/idc/bin/crtsurfdata /project/idc/ini/stcode.ini /tmp/idc/surfdata /log/idc/crtsurfdata.log csv,xml,json\n";

        cout<<"本程序是服务程序的调度程序，周期性启动服务程序或shell脚本。\n";
        cout<<"timetvl 运行周期，单位：秒。\n";
        cout<<"        被调度的程序运行结束后，在timetvl秒后会被procctl重新启动。\n";
        cout<<"        如果被调度的程序是周期性的任务，timetvl设置为运行周期。\n";
        cout<<"        如果被调度的程序是常驻内存的服务程序，timetvl设置小于5秒。\n";
        cout<<"program 被调度的程序名，必须使用全路径。\n";
        cout<<"...     被调度的程序的参数。\n";
        cout<<"注意，本程序不会被kill杀死，但可以用kill -9强行杀死。\n\n\n";
        return -1;
    }

    //关闭信号和IO，本程序不希望被打扰，不处理退出信号。
