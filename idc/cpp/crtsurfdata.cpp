...
int main(int argc,char *argv[])
{
   ...
   // 3) 把站点观测数据存保到文件中。
   if(strstr(argv[4],"csv")!=0) crtsurffile(argv[2],"csv");//如果数据文件格式中包含了csv就生成csv文件
...
bool loadstcode(const string &inifile)
{
   ...
   {
      logfile.write("ifile.open(%s) failed.\n",inifile.c_str());return false;//注意要转换成c风格的字符串
   }
...
bool crtsurffile(const string& outpath,const string& datafmt)
{
   //拼接生成数据的文件名，例如：tmp/idc/surfdata/SURF_ZH_20210629092200 2254.csv
   ...
   {
      logfile.write("11ofile.open(%s) failed.\n",strfilename.c_str()); return false;
   }
